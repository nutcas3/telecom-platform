...
impl RatingPlansRepo {
    ...
    pub async fn connect(database_url: &str) -> ChargingResult<Self> {
        ...
        // Initialize circuit breaker for PostgreSQL operations
        ...
        let postgres_cb_timeout: u64 = std::env::var("POSTGRES_CB_TIMEOUT")
            ...
            .and_then(|s| s.parse().ok())
            .unwrap_or(60);
        let circuit_breaker = CircuitBreaker::new(postgres_cb_threshold, std::time::Duration::from_secs(postgres_cb_timeout));
